    ...
    <div class="col-xs-12">
      <div class="box box-success"> 
        <div class="box-header">
          ...
          <div class="box-tools">
            <a href="{{ url('admin/article/category/create') }}"><button type="button" class="btn btn-block btn-success btn-sm" id="addUser">新建分类</button></a>
          </div>
        </div>
        <!-- /.box-header -->
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover">
            ...
            <td><input type="checkbox" class="check" value="{{$list->id}}"></td>
              <td>{{$list->id}}</td> 
              <td>{{$list->name}}</td>            
              <td><img src="{{ url('')}}/{{ $list->img }}" alt="图标" width="30px" height="30px"></td>
              <td>{{$list->desc}}</td>
              <!--  <td>John Doe</td> -->
              <td><div style="color: #dd4b39"><a href="{{url('admin/article/category')}}/{{$list->id}}/edit"><i class="fa fa-edit" style="margin-right: 5px;cursor: pointer;"></i></a><i class="fa fa-trash-o"  onclick="del({{$list->id}});" style="margin-right: 5px;cursor: pointer;"></div></i>
              </td>
            </tr>
            @endforeach
           <tr>
